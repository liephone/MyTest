...
void _tmain (int argc, TCHAR *argv[])
{
   ...
   if (fRemoveDriveLetter)
   {
	  ...
	  // test
	  _tprintf(_TEXT("delete fResult: %d\n"), fResult);
   ...
   else
   {
      ...
      fResult = DefineDosDevice (DDD_RAW_TARGET_PATH, szDriveLetter,
                                 pszNTDevice);
	  // test
	  _tprintf(_TEXT("DosDevice fResult: %d\n"), fResult);
	  _tprintf(_TEXT(" - szDriveLetter: %s\n"), szDriveLetter);
	  _tprintf(_TEXT(" - pszNTDevice: %s\n"), pszNTDevice);
      ...
      if (fResult)
      {
                  ...
                  MAX_PATH))
         {
            ...
            DEBUG_PRINT(szDriveLetterAndSlash, 0);
            DEBUG_PRINT(szUniqueVolumeName, 0);
         }

	  
		 // test
		 _tprintf(_TEXT("szDriveLetterAndSlash: %s\n"), szDriveLetterAndSlash);
		 _tprintf(_TEXT("szUniqueVolumeName: %s\n"), szUniqueVolumeName);
		 ...
		 // test
		 _tprintf(_TEXT("DosDevice 2 fResult: %d\n"), fResult);
		 ...
		 // test
		 _tprintf(_TEXT("SetVolumeMountPoint fResult: %d\n"), fResult);
